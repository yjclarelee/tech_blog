...
function renderPost(app, postId) {
  ...
  `;
    // Like
    const likeBtn = app.querySelector(".like-btn");
    const likeCount = app.querySelector(".like-count");
...
function renderAdminSetup(app) {
  ...
  `;
    ...
    const pw1 = document.getElementById("setup-pw");
...
function renderAdminLogin(app, admin) {
    app.innerHTML = `
    <main class="container admin-section">
      <h2>Admin Login</h2>
      <div class="admin-login">
...
function showPostEditor(app, existing) {
  ...
  `;
    ...
    form.addEventListener("submit", (e) => {
        ...
        const readTime = document.getElementById("ed-readtime").value.trim() || "3 min read";
...
function postCardHTML(post) {
    const excerpt = post.body.length > 180 ? post.body.slice(0, 180) + "…" : post.body;
    return `
...
function bindCommentForm(container, post, posts) {
...
function bindCommentDelete(container, post, posts) {
    const list = container.querySelector(".comments-list");
    list.addEventListener("click", (e) => {
        const target = e.target;
        ...
        if (!comment)
            return;
        showDeleteModal(async (password) => {
